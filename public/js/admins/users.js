$(document).ready(() => {
  ...
  $("#filterBtn").click((e) => {
    ...
    switch (option) {
      case "qruser":
        dataTable.ajax.url(`/api/admin/users/filter?sortBy=qruser`).load();
        break;
      default:
        break;
    }
  ...
  dataTable.on("click", "tbody > tr > td > div > div > span", function (e) {
    ...
    axios
      .put(`/api/admin/users/${data._id}/approved`, dumpData, { headers })
      .then((response) => {
        const codeData = {
          owner: data._id,
        };
        axios
          .post("/api/admin/codes", codeData, { headers })
          .then((response) => {
            ...
            axios
              .post("/api/admin/msg-service/sms/send", SMSData, { headers })
              .then((response) => {
                $("#successModal").modal("show");
                $("#successModalTitle").html("User approval succeeded");
                $("#successModalContent").html(
                  "Your approval of user is success with a request code for machine"
                );
      ...
      .catch((e) => {
        ...
        $("#errorModalContent").html(
          "Your approval of user is failed. Please contact support."
        );
      });
  });
  ...
  dataTable.on("click", "td.details-control", function () {
    ...
    axios
      .get(`/api/admin/codes/user/${data._id}`, { headers })
      .then((response) => {
        data = response.data;
  ...
  $("#userDelForm").submit((e) => {
    ...
    if (confirm.toLowerCase() === "confirm") {
      axios
        .delete(`/api/admin/users/${currentData._id}`, { headers })
        .then((response) => {
        ...
        .catch((e) => {
          ...
          $("#errorModalContent").html(
            "The process of deleting user is failed. Please check your data and retry again."
          );
        });
    }
  });
...
const format = (data) => {
  ...
  return (
    '<div class="row"><div class="col-md-4"><table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">' +
    ...
    "<td>" +
    expiredDateFormat(data.createdAt) +
    "</td>" +
    "</tr>" +
    "</table></div></div>"
  );
